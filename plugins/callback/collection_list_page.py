...
def generate_page(request: CollectionsRequest) -> CollectionsRequest:
    ...
    try:
        response = requests_get(
            url=url, params=params, access_token=request.session.bgm_auth['access_token']
        )
    ...
    for info, num, nums_unicode in zip(subject_list, nums, nums_unicode):
        ...
        text_data += (
            f'*{nums_unicode}* {info["subject_info"]["name_cn"] or info["subject_info"]["name"]}'
        ...
        request.possible_request[nums_unicode] = SubjectRequest(
            request.session, info['subject_id']
        )
    text = (
    ...
    if count > limit:
        ...
        if offset - limit >= 0:
            button_list2.append(
                telebot.types.InlineKeyboardButton(
                    text='上一页', callback_data=f'{session_uuid}|back'
                )
            )
        ...
        if offset + limit < count:
            button_list2.append(
                telebot.types.InlineKeyboardButton(
                    text='下一页', callback_data=f'{session_uuid}|{offset + limit}'
                )
            )
