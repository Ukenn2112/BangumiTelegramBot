...
def generate_page(week_request: WeekRequest, stack_uuid: str) -> WeekRequest:
    week_data = get_calendar()
    if week_data is None:
        raise RuntimeError("出错了")
    for i in week_data:
        if i.get('weekday', {}).get('id') == int(week_request.week_day):
            items = i.get('items')
            ...
            for item, num in zip(items, nums):
                ...
                button_list.append(telebot.types.InlineKeyboardButton(
                    text=str(num), callback_data=f"{stack_uuid}|{item['id']}"))  # TODO
            ...
            for week_day in range(1, 8):
                ...
                week_request.possible_request[day_str] = WeekRequest(week_day)
            ...
            week_request.page_text = text
            week_request.page_markup = markup
    return week_request
