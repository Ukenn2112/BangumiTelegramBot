...
def generate_page(request: CollectionsRequest, stack_uuid: str) -> CollectionsRequest:
    ...
    for info, num, nums_unicode in zip(subject_list, nums, nums_unicode):
        ...
        request.possible_request[nums_unicode] = SubjectRequest(str(info['subject_id']))
    if subject_type == 1:
    ...
    if count > limit:
        ...
        if offset - limit >= 0:
            ...
            request.possible_request[str(offset - limit)] = CollectionsRequest(request.user_data, subject_type,
                                                                               offset=offset - limit,
