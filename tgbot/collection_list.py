...
async def send_collection_list(message: Message, bot: AsyncTeleBot):
    ...
    text_sp: List[str] = message.text.split(" ")
    subject_types: Dict[str, Literal[1, 2, 3, 4, 6]] = {
        "/book": 1,
        "/anime": 2,
        "/music": 3,
        "/game": 4,
    ...
    subject_type: Literal[1, 2, 3, 4, 6] = 2
    for key, value in subject_types.items():
        if text_sp[0].lstrip().startswith(key):
            subject_type = value
            break
    ...
    if user_data is None:
        ...
        return await bot.reply_to(message,
                                  f"发现您未绑定 Bangumi，快*[点我](https://t.me/{bot_data.username}?start=None)*进行绑定吧～",
                                  parse_mode="MarkdownV2")
    msg = await bot.reply_to(message, "正在获取收藏列表，请稍候...")
    collection_type = 3
    if message.text:
        if len(text_sp) > 1:
